...
class ReapContext(HarvestContext):
    ...
    def reap_higher_order_primitive(
        self, trace, call_primitive, f, tracers, params, is_map
    ):
...
def reap_function(
    ...
    with staging.new_dynamic_context(main, context):
        ...
        del main
    out_values, reap_values = tree_util.tree_map(
        lambda x: x.val, (out_tracers, reap_tracers)
    )
    if return_metadata:
...
def reap_eval(
    f: lu.WrappedFun, trace: HarvestTrace, settings: HarvestSettings
...
def _get_harvest_metadata(closed_jaxpr, settings, *args):
    ...
    with jax_core.new_main(HarvestTrace) as main:
        ...
        in_avals = jax_util.safe_map(
            lambda a: abstract_arrays.raise_to_shaped(jax_core.get_aval(a)), flat_args
        )
...
def _reap_scan_rule(
    ...
    *tracers,
    ...
    (
        ...
        consts,
        out_tree,
    ) = lcf._initial_style_jaxpr(  # pylint: disable=protected-access
        new_body,
        reap_carry_in_tree,
    ...
    (carry_out, carry_reaps), (ys, ys_reaps) = tree_util.tree_unflatten(out_tree, out)
...
def _reap_while_rule(
    ...
    _, init_avals = tree_util.tree_map(
        lambda x: x.aval, (body_const_tracers, init_tracers)
    )
    ...
    dummy_reap_vals = tree_util.tree_map(
        lambda x: jnp.zeros(x.shape, x.dtype), reap_avals
    )
...
def _reap_cond_rule(trace, *tracers, branches, linear):
    ...
    index_val, ops_vals = tree_util.tree_map(
        lambda x: x.val, (index_tracer, ops_tracers)
    )
    _, ops_avals = tree_util.tree_map(lambda x: x.aval, (index_tracer, ops_tracers))
    context = staging.get_dynamic_context(trace)
    settings = context.settings
    reap_settings = dict(
        tag=settings.tag,
...
def _reap_checkpoint_rule(trace, *tracers, jaxpr, policy, prevent_cse, differentiated):
    ...
    (
        ...
        reaped_remat_fun,
        tree_util.tree_structure(invals),
...
def _reap_pjit_rule(trace, *tracers, **params):
    ...
    if params["in_shardings"] and not any(
        pjit._is_unspecified(i) for i in params["in_shardings"]
    ):  # pylint: disable=protected-access
        raise ValueError(
    ...
    if params["out_shardings"] and not any(
        pjit._is_unspecified(o) for o in params["out_shardings"]
    ):  # pylint: disable=protected-access
        raise ValueError(
    ...
    new_params = {
        **params,
        "jaxpr": reap_jaxpr,
        "out_shardings": (pjit._UNSPECIFIED,)
        * len(out_avals),  # pylint: disable=protected-access
...
class PlantContext(HarvestContext):
    ...
    def process_higher_order_primitive(
        ...
        if "donated_invars" in params:
            params = dict(params)
            params["donated_invars"] = (False,) * len(
                tree_util.tree_leaves(plants)
...
def plant(
    ...
    *,
    ...
    """Transforms a function into one that injects values in place of sown
    ones.
...
def _plant_scan_rule(
    ...
    *tracers,
    ...
    carry_avals, xs_avals = tree_util.tree_map(
        lambda x: x.aval, (carry_tracers, xs_tracers)
    )
    const_vals, carry_vals, xs_vals = tree_util.tree_map(
        lambda x: x.val, (const_tracers, carry_tracers, xs_tracers)
    )
