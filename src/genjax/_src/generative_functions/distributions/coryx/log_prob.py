...
class LogProbRules(dict):
    ...
    By default, the rules for LogProb propagation are just the
    InverseAndILDJ rules, but instead of raising a NotImplementedError,
    LogProb will silently fail. This default dict-like class implements
    this behavior, but also allows primitives to register custom
    propagation rules.
    """
...
def log_prob_jaxpr(jaxpr, constcells, flat_incells, flat_outcells):
    ...
    def reducer(env, eqn, curr_log_prob, new_log_prob):
        if isinstance(curr_log_prob, FailedLogProb) or isinstance(
            new_log_prob, FailedLogProb
        ):
...
def make_default_rule(prim):
    ...
    def rule(incells, outcells, **params):
        """Executes the inverse rule but fails if the inverse isn't
        implemented."""
        try:
